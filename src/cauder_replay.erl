...
replay_start(#sys{nodes = Nodes, logs = LMap} = Sys, Node) ->
  ...
  case NodeExists of
    true -> Sys;
    false ->
      NewSys = case {FutureReads, FailedSpawns} of
                 {{value, ProcWithRead}, _} -> replay_step(Sys, ProcWithRead);
                 {_, {value, ProcWithFailedSpawn}} -> replay_step(Sys, ProcWithFailedSpawn);
                 {_, _} ->
