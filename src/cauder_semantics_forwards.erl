...
-spec step(System, Pid, MessageScheduler) -> NewSystem when
  System :: cauder_types:system(),
  Pid :: cauder_types:proc_id(),
  MessageScheduler :: cauder_types:message_scheduler(),
  NewSystem :: cauder_types:system().

step(#sys{mail = Mail, logs = LMap, trace = Trace} = Sys, Pid, Sched) ->
    ...
    {rec, VarBody, Cs} when Es == [VarBody] ->
      {{Bs1, Es1, {Msg, QPos}, Mail1}, NewLog} =
        case LMap of
          ...
          _ ->
            {cauder_eval:match_rec_pid(Cs, Bs, Pid, Mail, Sched, Sys), []} % TODO Refactor
        end,
...
options(#sys{mail = Mail, procs = PMap, logs = LMap} = Sys) ->
  lists:foldl(
    fun
      (#proc{pid = Pid, stack = Stk, env = Bs, exprs = Es}, Opts) ->
        Log = maps:get(Pid, LMap, []),
        case expression_option(Pid, Es, Bs, Stk, Log, Mail, Sys) of
          ?NOT_EXP -> Opts;
          Rule ->
            Opt = #opt{
              sem  = ?MODULE,
...
-spec expression_option(Pid, Expressions, Environment, Stack, Log, Mail, Sys) -> Rule when
  Pid :: cauder_types:proc_id(),
...
expression_option(Pid, E, Bs, Stk, Log, Mail, Sys) when not is_list(E) ->
  expression_option(Pid, [E], Bs, Stk, Log, Mail, Sys);
expression_option(Pid, [E0 | Es0], Bs, Stk, Log, Mail, Sys) ->
      ...
      case E0 of
        ...
        {cons, _, H, T} ->
          case is_reducible(H, Bs) of
            true -> expression_option(Pid, H, Bs, Stk, Log, Mail, Sys);
            false -> expression_option(Pid, T, Bs, Stk, Log, Mail, Sys)
          end;
        {tuple, _, Es} -> expression_option(Pid, Es, Bs, Stk, Log, Mail, Sys);
        {'if', _, _} -> ?RULE_SEQ;
        {'case', _, E, _} ->
          case is_reducible(E, Bs) of
            true -> expression_option(Pid, E, Bs, Stk, Log, Mail, Sys);
            false -> ?RULE_SEQ
          end;
        {'receive', _, Cs} ->
          IsMatch =
            case Log of
              [] -> cauder_eval:match_rec_pid(Cs, Bs, Pid, Mail, ?SCHEDULER_RoundRobin, Sys) =/= nomatch;
        ...
        {bif, _, _, _, As} ->
          case is_reducible(As, Bs) of
            true -> expression_option(Pid, As, Bs, Stk, Log, Mail, Sys);
            false -> ?RULE_SEQ
          end;
        {self, _} -> ?RULE_SELF;
        {spawn, _, F} ->
          case is_reducible(F, Bs) of
            true -> expression_option(Pid, F, Bs, Stk, Log, Mail, Sys);
            false -> ?RULE_SPAWN
          end;
        {spawn, _, M, F, As} ->
          case is_reducible(M, Bs) of
            true -> expression_option(Pid, M, Bs, Stk, Log, Mail, Sys);
            false ->
              case is_reducible(F, Bs) of
                true -> expression_option(Pid, F, Bs, Stk, Log, Mail, Sys);
                false ->
        ...
        {Send, _, L, R} when Send =:= 'send' orelse Send =:= 'send_op' ->
          case is_reducible(L, Bs) of
            true -> expression_option(Pid, L, Bs, Stk, Log, Mail, Sys);
            false ->
              case is_reducible(R, Bs) of
                true -> expression_option(Pid, R, Bs, Stk, Log, Mail, Sys);
                false -> ?RULE_SEND
              end
          end;
        {local_call, _, _, As} ->
        ...
        {apply_fun, _, Fun, As} ->
          case is_reducible(Fun, Bs) of
            true -> expression_option(Pid, Fun, Bs, Stk, Log, Mail, Sys);
            false ->
              case is_reducible(As, Bs) of
                true -> expression_option(Pid, As, Bs, Stk, Log, Mail, Sys);
                false -> ?RULE_SEQ
          ...
          case is_reducible(E, Bs) of
            ...
            false ->
              case is_reducible(P, Bs) of
                true -> expression_option(Pid, P, Bs, Stk, Log, Mail, Sys);
                false -> ?RULE_SEQ
              end
          end;
        {op, _, _, Es} ->
          case is_reducible(Es, Bs) of
            true -> expression_option(Pid, Es, Bs, Stk, Log, Mail, Sys);
            false -> ?RULE_SEQ
          end;
