...
options(#sys{mail = Mail, procs = PMap, logs = LMap} = Sys, Mode) ->
    lists:foldl(
        fun(#proc{pid = Pid, stack = Stk, env = Bs, exprs = Es}, Opts) ->
    ...
    case is_reducible(E0, Bs) of
        ...
        true ->
            case E0 of
                ...
                {tuple, _, Es} ->
                    expression_option(Pid, Es, Bs, Stk, Log, Mail, Sys, Mode);
                {'if', _, _} ->
                    ?RULE_SEQ;
                {'case', _, E, _} ->
                ...
                {spawn, _, M, F, As} ->
                    case is_reducible(M, Bs) of
                        true ->
                            expression_option(Pid, M, Bs, Stk, Log, Mail, Sys, Mode);
                        false ->
                            case is_reducible(F, Bs) of
                                true ->
                                    expression_option(Pid, F, Bs, Stk, Log, Mail, Sys, Mode);
                                false ->
                                    case is_reducible(As, Bs) of
                                        true -> expression_option(Pid, As, Bs, Stk, Log, Mail, Sys, Mode);
                                        false -> ?RULE_SPAWN
                ...
                {Send, _, L, R} when Send =:= 'send' orelse Send =:= 'send_op' ->
                    case is_reducible(L, Bs) of
                        true ->
                            expression_option(Pid, L, Bs, Stk, Log, Mail, Sys, Mode);
                        false ->
                ...
                {apply_fun, _, Fun, As} ->
                    case is_reducible(Fun, Bs) of
                        true ->
                            expression_option(Pid, Fun, Bs, Stk, Log, Mail, Sys, Mode);
                        false ->
                            case is_reducible(As, Bs) of
                                true -> expression_option(Pid, As, Bs, Stk, Log, Mail, Sys, Mode);
                                false -> ?RULE_SEQ
