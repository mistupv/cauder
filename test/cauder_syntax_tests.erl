...
pattern_test_() ->
    [
        ...
        ?_assertPattern(
            {match, 1, {tuple, 1, [{var, 1, 'A'}, {var, 1, 'B'}]}, {value, 1, {answer, 42}}},
            "{A, B} = {answer, 42}"
        ),
        ?_assertPattern(
            {match, 1, {tuple, 1, [{var, 1, 'C'}, {var, 1, 'D'}]}, {value, 1, [1, 2]}},
...
guard_test_test_() ->
    [
        ...
        % operators
        ?_assertGuardTest(
            {'orelse', 1, {bif, 1, erlang, is_atom, [{var, 1, 'L'}]},
                {'andalso', 1, {bif, 1, erlang, is_list, [{var, 1, 'L'}]},
                    {op, 1, '==', [
...
expr_test_() ->
    {setup,
        ...
        [
            ...
            % TODO compile-time expressions
            ?_assertExpr({op, 1, '-', [{value, 1, 1234567890}]}, "-1_234_567_890"),
            ...
            ?_assertExpr(
                {'receive', 1, [
                    {clause, 2, [{value, 2, onhook}], [], [
                        {local_call, 3, disconnect, []},
                        {local_call, 4, idle, []}
                    ]},
                    {clause, 5, [{tuple, 5, [{value, 5, connect}, {var, 5, 'B'}]}], [], [
            ...
            ?_assertExpr(
                {make_fun, 1, '-sample/0-fun-0-', [
                    {clause, 1, [{var, 1, 'X'}], [], [{op, 1, '+', [{var, 1, 'X'}, {value, 1, 1}]}]}
                ]},
                "fun (X) -> X + 1 end"
            ),
            ?_assertExpr(
                {make_fun, 1, '-sample/0-fun-1-', [
                    {clause, 1, [{var, 1, 'X'}], [[{op, 1, '>=', [{var, 1, 'X'}, {value, 1, 5}]}]], [{value, 1, gt}]},
