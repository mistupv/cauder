...
main() -> MePid = spawn(?MODULE,meManager,[]), 
          XPid = spawn(?MODULE,varManager,[0]),
          spawn(?MODULE,incrementer,[MePid,XPid]),
          spawn(?MODULE,incrementer,[MePid,XPid]).
 
meManager() ->
    receive {request,Pid} -> Pid!answer end,
    receive {release} -> meManager() end.

varManager(Val) ->
    io:format("Variable value:~b~n",[Val]),
    receive {write,NewVal} -> varManager(NewVal);
    	    {read,Pid} -> Pid!Val end,
    varManager(Val).
