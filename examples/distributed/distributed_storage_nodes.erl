                ...
                {ok, full} ->
                    ...
                    case StorageCounter rem 5 =:= 0 of
                        true ->
                            ...
                            %bug
                            slave:start(SelectedDomain, list_to_atom(atom_to_list(storageNode) ++ "1")),
                            NewStorageNode = list_to_atom(
                                atom_to_list(storageNode) ++ "1@" ++ atom_to_list(SelectedDomain)
                            ),
                            NewStorageManager = spawn(NewStorageNode, ?MODULE, storageManager, [[]]),
                            NewDomainIndex = DomainIndex rem 3 + 1,
                            NewDomain = list_to_atom(
                                Domain ++ "." ++ (lists:flatten(io_lib:format("~p", [CounterInt + 1])))
                            ),
                            NewDomains =
                                lists:sublist(Domains, DomainIndex - 1) ++ [NewDomain] ++
                                    lists:sublist(Domains, DomainIndex + 1, length(Domains) + 1),
                            Client ! ok,
                            server(NewDomains, NewDomainIndex, NewStorageManager, 1);
                        false ->
                            {ok, NewNode} = slave:start(
                                list_to_atom(atom_to_list(SelectedDomain)),
                                list_to_atom(
                                    atom_to_list(storageNode) ++
                                        (lists:flatten(io_lib:format("~p", [StorageCounter + 1])))
                                )
                            ),
